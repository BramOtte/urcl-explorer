...
client.on("messageCreate", (msg) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield msg.reply({ "content": err_msg.substring(0, max_total) });
    }
}));
    ...
    return __awaiter(this, void 0, void 0, function* () {
        if (msg.content.toLowerCase().includes("!lol")) {
            ...
            if (text.length > max_msg) {
                yield msg.reply("Message too large");
                return;
            }
        ...
        if (msg.content.includes(urcl_start)) {
            ...
            for (const line of lines) {
                if (line.length + 1 + ansi_start.length + code_block_end.length > max_total) {
                    if (lex.length > 0) {
                        msg = yield msg.reply({ content: `${ansi_start}${lex}${code_block_end}` });
                    }
                    lex = "";
                    msg = yield msg.reply({ content: `Line too long starting with: ${line.substring(0, 20)}` });
                    continue;
                }
        ...
        if (content.startsWith("!urclpp")) {
            ...
            if (source === undefined) {
                yield msg.reply("no source specified");
                return;
            }
            const { code, out, errors } = yield my_exec("python3", "URCLpp-compiler/compiler2.py", `imm:${source}`);
            const rep_msg = `exit code ${code}` + (errors ? `\nerrors: \`\`\`\n${errors}\`\`\`` : "");
            msg = yield reply_text(msg, rep_msg);
            msg = yield reply_text(msg, out);
            ...
            let source = parse_code_block(content);
            if (!source) {
                const source_attach = msg.attachments.find(v => { var _a, _b; return !!((_b = (_a = v.name) === null || _a === void 0 ? void 0 : _a.endsWith) === null || _b === void 0 ? void 0 : _b.call(_a, ".urcl")); });
                if (source_attach) {
            ...
            yield msg.reply({ files: [new MessageAttachment(Buffer.from(out, "utf8"), "output.txt")], content: rep_msg });
        }
        ...
        function reply(msg, res) {
            return __awaiter(this, void 0, void 0, function* () {
                let { out, info, screens, all_screens, scale, state, quality, storage } = yield res;
                let files = [];
                let screen_at;
                ...
                if (to_draw.length > 0) {
                    ...
                    const max_images = 0 | 1000000 / (width * height);
